...
>
> **最后更新：2026-09-18**（新增 §2.3 犀牛 Boss；§一 计数与 §十 清单同步）
...
### 2.3 犀牛（Rhino，Boss）精灵图

| 项目 | 内容 |
|---|---|
| 游戏内文件 | `resource/sprite/role/rhino/` 下 8 张横条图（每张 271×77 × N 帧）：`RhinoMoveL.png`(9) / `RhinoCharge.png`(8) / `RhinoBurrow.png`(7) / `RhinoBiteS.png`(8) / `RhinoBiteL.png`(8) / `RhinoDash.png`(8) / `RhinoSweep.png`(8) / `RhinoSlam.png`(7)<br>`rhino_frames.json`（切帧几何记录）<br>`prefab/role/enemy/Rhino.tscn` |
| **AI 源图** | `C:\Users\WY157\Documents\Codex\2026-09-17\c-users-wy157-desktop-gunfiredungeon-1999\outputs\rhino-boss-overview-10x10-cute-white.png`（1024×1024，品红背景，无 alpha）|
| **工具** | **Codex**（OpenAI 的编码代理），产出目录 `Documents\Codex\<日期>\...\outputs\`<br>**⚠️ 具体图像模型与生成日期待补**（该目录里同批的 `pixel-art-*` 文件与 §2.2 同源，疑为 `gpt-image-2`）|
| 日期 | 2026-09-17 21:48（文件时间戳）|
| 同批**未接入**的中间产物 | `rhino-boss-overview-10x10-100-frames.png`、`rhino-boss-four-new-skills-6x4.png`、`rhino-boss-attacks-4-actions-6-frames.png`、`rhino-boss-quadruped-walk-spritesheet-4x4.png`、`rhino-walk-spritesheet-4x4.png` —— **全部未接入游戏，不进发行包** |
| **人类二次创作** | ① **品红抠图**：源图背景是**抖动**的品红（250/2/248 及其邻域，不是纯色），按「品红程度 = min(R,B) − G」做硬色键 + **despill 去溢色**。<br>　　第一版用 unmatte 反解公式，会把抗锯齿过渡带推成品红（**19% 的前景像素偏品红**，犀牛描边变粉）；换硬键 + despill 后残留 **0.000%**。<br>　　另外**不能用闭运算补洞** —— 会把土石之间透出的背景品红填成前景（一堆粉点）。<br>② **按连通域切帧**，没有用 10×10 固定栅格：实测每行帧数是 **9/9/8/-/7/8/8/8/8/7**（不是 10），而且尘土/点阵会把相邻格连成一片（出现过 367、603 像素宽的连通段）；按 102.4 硬切会把第一个精灵（x 16~107）切成两半。<br>③ 统一画布 **271×77** + 计算「脚下中心」锚点 **(136,74)** 并让所有帧共用（否则播放会抖）；画布高度**排除弃用的第 4 行**（它那 85px 高的尘土会把画布撑大）。<br>④ 导出 8 张横条图 + 写 `rhino_frames.json`。<br>⑤ 编写 `RhinoSpriteFrames.cs`（用 `AtlasTexture` 装配 10 个动画）与 `RhinoEnemy.cs`（7 个技能 + 常态碰撞伤害）。<br>⑥ 受击框按身体实测尺寸重设（**88×56**，身体实测 86~90 × 54~60）。<br>⑦ 判定「第 5 行的土石段本身就是『犀牛在地下』的帧」，据此设计掘地技能，不需要另画素材。 |
| 处理脚本 | `_music/rhino_extract_frames.py`（**仓库外**，与音乐处理脚本同目录）|
| 披露状态 | ✅ 已记录 |

> **朝向说明（值得记一笔）**：源图所有帧都**朝左**。看起来「Rhino 向右时要把技能动画镜像」，
> 但本引擎 `Role.Face` 的 setter → `_SetFace()` 会把**整个节点**旋转 180° + 翻转 Y，
> 也就是**引擎已经自动做了水平镜像**。所以只装载朝左的那一套即可，
> 原图**第 2 行（朝右那套）故意弃用** —— 用了会翻两次，变成反的。
...
| 项目 | 内容 |
|---|---|
| 范围 | 2026-09-14 起持续至今的提交，涉及 BGM 系统、商店修复、魔王模式、翻滚操作、地图守卫、设置菜单、弹夹（备用弹药）系统、伤害计算、Boss（大橘 / Rhino）等 |
...
| `_music/` | 仓库外 | 音乐处理脚本与母版；**犀牛切图脚本 `rhino_extract_frames.py` 也在这里** |
| `_tools/` | 仓库外 | 各类处理脚本 |
| `C:\Users\WY157\Documents\Codex\` | 仓库外 | **Codex 的工作目录**，犀牛源图与同批中间产物都在这里（§2.3）|
